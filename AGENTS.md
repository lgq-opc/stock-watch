# stock-watch 写入约定

- 仓库用途：港股 / AI 相关标的盯盘归档。
- 观察名单：`config/watchlist.md`（用户或助手可改）。
- 每波扫描后：写入 `notes/YYYY-MM-DD.md`（按 08:00 / 10:00 / 12:30 / 14:30 分节追加），并 push 到 `origin`（`lgq-opc/stock-watch`）。
- 向用户汇报：简体中文、可扫读；休市日可静默。
- Git 作者：小青（Grok Bot）。

## 三市下午总结

- 工作日约 16:44（Asia/Shanghai，港股收盘后）生成 `notes/YYYY-MM-DD-daily-summary.md`
- 覆盖 A股收盘 + 港股收盘 + 美股昨夜（及盘前如有）
- 必覆盖 `config/watchlist.md` Tier-1；Tier-2 仅异动
- push 后向用户发简体中文总结

## 技能

盘中四波与三市下午总结均执行共享技能「股市盯盘与三市总结」（`skill-1790146043654`）。
自进化写入 `config/evolution.md`；连续摩擦再考虑改技能正文。
每波/每日发出前须过技能内质量自检与审阅门闩。

## 技能双份同步

- 仓库：`skills/股市盯盘与三市总结/SKILL.md`
- 技能库：`skill-1790146043654`（名称：股市盯盘与三市总结）
- 两份正文必须一致；例行任务执行时**优先 Read 仓库这份**，技能库保留不删。
- 进化运行日志只写 `config/evolution.md`，不要每轮改 SKILL 正文。
